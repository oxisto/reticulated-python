...
class ArithmeticTest {
  ...
  fun multiplicativeToStringTest() {
	...
	unaryOperator=POSITIVE UnaryExpr=FloatNumber(
	value=0.5
)
) binaryOperator=MULTIPLICATION unaryExpr=UnaryExpr(
	unaryOperator=NEGATIVE UnaryExpr=BytesLiteral(
	value=1
)
)
), MultiplicativeExpr(
	multiplicativeExpr=BytesLiteral(
	value=0
) binaryOperator=DIVISION unaryExpr=ImagNumber(
	value=FloatNumber(
	value=0.1
) "j"
)
	...
	multiplicativeExpr=BytesLiteral(
	value=1
) binaryOperator=FLOOR_DIVISION unaryExpr=FloatNumber(
	value=0.5
)
), MultiplicativeExpr(
	multiplicativeExpr=Integer(
	value=1
  ...
  fun arithmeticToStringTest() {
	...
	additiveExpr=Integer(
	value=1
) binaryOperator=ADDITION multiplicativeExpr=UnaryExpr(
	unaryOperator=NEGATIVE UnaryExpr=UnaryExpr(
	unaryOperator=NEGATIVE UnaryExpr=UnaryExpr(
	unaryOperator=POSITIVE UnaryExpr=UnaryExpr(
	unaryOperator=NEGATIVE UnaryExpr=Integer(
	value=1
)
)
)
	...
	additiveExpr=FloatNumber(
	value=0.5
) binaryOperator=ADDITION multiplicativeExpr=ImagNumber(
	value=Integer(
	value=1
) "j"
)
	...
	additiveExpr=StringLiteral(
	value=Test
) binaryOperator=ADDITION multiplicativeExpr=StringLiteral(
	value=Test
)
)
), AssignmentExpression(
	...
	unaryOperator=BITWISE_NOT UnaryExpr=UnaryExpr(
	unaryOperator=BITWISE_NOT UnaryExpr=BytesLiteral(
	value=11
)
)
) binaryOperator=SUBTRACTION multiplicativeExpr=ImagNumber(
	value=FloatNumber(
	value=0.1
) "j"
)
